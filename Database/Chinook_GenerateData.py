...
def generate_systemlog(count=5000):
    ...
    Args:
        count: Number of log entries (each ~100KB after SQL padding, so 5000 rows ≈ 500MB)
    ...
    # No padding in Python - SQL Server will generate it using REPLICATE() during insertion
    # This keeps Python memory low and SQL file small while still creating large DB
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        print("  SQL Server generates ~100KB padding per row during insertion (fast & memory-efficient)")
    ...
    if generate_systemlog_data:
        estimated_mb = (systemlog_count * 100) // 1000  # ~100KB per row after SQL padding
        ...
        print(f"✓ Generated {len(systemlog):,} log entries (SQL Server will add ~100KB padding per row)")
        print()
    else:
        systemlog = []
...
def write_mssql_format(f, artists, albums, tracks, customers, invoices, invoice_lines, systemlog):
    ...
    if systemlog and len(systemlog) > 0:
        ...
        f.write("-- SQL Server generates ~100KB padding per row using REPLICATE() for fast insertion\n")
        ...
        for batch_num in range(0, len(systemlog), batch_size):
            ...
            f.write("BEGIN\n")
            f.write("INSERT INTO [dbo].[SystemLog] ([Timestamp], [LogLevel], [Message])\n")
            f.write("SELECT [Timestamp], [LogLevel], [Message] + ' | ' + REPLICATE('PADDING_', 10000) FROM (\n")
            f.write("VALUES\n")
            ...
            f.write(") AS LogData([Timestamp], [LogLevel], [Message]);\n")
            f.write("END\n")
            f.write("GO\n\n")
