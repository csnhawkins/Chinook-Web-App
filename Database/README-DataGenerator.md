...
- ~50KB padding per row (generated by SQL Server during insertion)
- Timestamps match invoice date range (2022-2026)
- **Sizing**: 2,000 rows ≈ 100MB | 10,000 rows ≈ 500MB | 20,000 rows ≈ 1GB
...
Generating 10,000 SystemLog entries (~500MB after SQL padding)...

✓ Generated 10,000 log entries (SQL Server will add ~50KB padding per row)
...
Generate SystemLog entries for database size inflation?
  SQL Server generates ~50KB padding per row during insertion (fast & memory-efficient)
  100MB ≈ 2,000 rows | 500MB ≈ 10,000 rows | 1GB ≈ 20,000 rows
